...
class Encryptor {
    ...
    fun encrypt(content: ByteArray): ByteArray? {
        val encryptedContent = encrypt(content, Cipher.ENCRYPT_MODE)
        val encodedContent = Base64.encode(encryptedContent, Base64.DEFAULT)
        Log.i(TAG, "Encrypt -> Encoded Base64")
        Log.i(TAG, String(encodedContent))
        return encodedContent
    }
    
    fun decrypt(content: ByteArray): ByteArray? {
        Log.i(TAG, "Decrypt -> Encoded Base64")
        Log.i(TAG, String(content))
        val decodedContent = Base64.decode(content, Base64.DEFAULT)
        return encrypt(decodedContent, Cipher.DECRYPT_MODE)
    }
